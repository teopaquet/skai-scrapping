...
class AirfleetScraper:
    ...
    def _get_browser(self, quiet):
        ...
        # Try using automatic driver management first
        try:
    ...
    def start(self, selected_types):
        ...
        time.sleep(2)
        aircraft_menu = self.browser.find_element(By.XPATH,
            "//button[@class='dropbtn' and text()='Aircraft				  ']")
        action.move_to_element(aircraft_menu).perform()
        aircraft_url = self.browser.find_element(By.XPATH, "//a[text()='Supported planes']").get_attribute('href')
        self._get_url_with_delay(aircraft_url)

        aircraft_type_urls = []
        for a_type_element in self.browser.find_elements(By.CLASS_NAME, "trtab"):
            child = a_type_element.find_element(By.CLASS_NAME, "lien")
        ...
        for type_name, type_url in aircraft_type_urls:
            ...
            while continue_next_page:
                airplanes_msn_urls.extend([msn.find_element(By.CLASS_NAME, "lien").get_attribute('href') for msn
                                           in self.browser.find_elements(By.CLASS_NAME, "trtab")])
                try:
                    next_page_url = self.browser.find_element(By.XPATH, "//a[@class='page' and text()='Next page ']").get_attribute('href')
